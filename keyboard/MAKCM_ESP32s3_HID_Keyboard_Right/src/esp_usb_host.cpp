...
uint8_t EspUsbHost::lastKeyboardBitmap[20] = {0};
    ...
    case USB_INTERFACE_DESC:
    {
        ...
        if (interfaceCounter < MAX_INTERFACE_DESCRIPTORS)
        {
            ...
            else
            {
                ...
                this->usbInterface[this->usbInterfaceSize].devHandle = this->deviceHandle;
                this->usbInterface[this->usbInterfaceSize].ifNum = intf->bInterfaceNumber;
...
void EspUsbHost::_clientEventCallback(const usb_host_client_event_msg_t *eventMsg, void *arg)
{
    ...
    case USB_HOST_CLIENT_EVENT_DEV_GONE:
    {
        ...
        deviceKeyboardReady = false;
        usbHost->mouseEndpointNum = 0xFF;
        usbHost->keyboardEndpointNum = 0xFF;
        usbHost->mouseDeviceHandle = NULL;
        usbHost->keyboardDeviceHandle = NULL;
        memset(lastKeyboardBitmap, 0, sizeof(lastKeyboardBitmap));
...
void EspUsbHost::cleanupTask(void *arg)
{
    ...
    while (true) {
        ...
        for (int i = 0; i < usbHost->usbInterfaceSize; i++) {
            err = usb_host_interface_release(usbHost->clientHandle, usbHost->usbInterface[i].devHandle, usbHost->usbInterface[i].ifNum);
            if (err == ESP_OK) {
        ...
        for (int i = 0; i < usbHost->openDeviceCount; i++) {
            if (usbHost->openDeviceHandles[i] != NULL) {
                err = usb_host_device_close(usbHost->clientHandle, usbHost->openDeviceHandles[i]);
                if (err == ESP_OK) {
                    ESP_LOGI("EspUsbHost", "Closed device at index %d", i);
                } else {
                    ESP_LOGE("EspUsbHost", "Failed to close device at index %d. Error: %d", i, err);
                }
                usbHost->openDeviceHandles[i] = NULL;
            }
        }
        usbHost->openDeviceCount = 0;
...
void EspUsbHost::_onReceiveControl(usb_transfer_t *transfer)
{
    ...
    {
        if (p[i] == 0x05 && p[i + 1] == 0x01 && p[i + 2] == 0x09)
        {
            if (p[i + 3] == 0x02) {
                isMouse = true;
                break;
    ...
    if (isMouse)
    {
        ...
        usbHost->mouseDeviceHandle = transfer->device_handle;
    ...
    {
        ESP_LOGI("EspUsbHost", "Keyboard device detected");
        ...
        {
            if (usbHost->endpoint_data_list[i].bInterfaceClass == USB_CLASS_HID &&
                usbHost->endpoint_data_list[i].bInterfaceProtocol == HID_ITF_PROTOCOL_KEYBOARD)
            {
                usbHost->keyboardEndpointNum = i;
                ESP_LOGI("EspUsbHost", "Keyboard endpoint cached at index %d", i);
                break;
            }
        }
    }
...
// NKRO bitmap covering keycodes 0x00-0x9F (must match the Left MCU device).
#define KB_BITMAP_BYTES  20
#define KB_KEY_MAX       0x9F
#define KB_MARKER        0xAB

void EspUsbHost::onKeyboard(const uint8_t *data, int length)
{
    if (!deviceKeyboardReady || length < 8) return;

    uint8_t modifiers = data[0];

    // Build the NKRO bitmap from the report's keycode array. data[0] is the
    // modifier byte and data[1] is reserved (boot layout); every remaining
    // byte is treated as a pressed HID usage code. Reading the whole array
    // (not just 6 slots) transparently supports array-style NKRO keyboards
    // that emit longer reports, while boot keyboards simply carry 6 codes.
    uint8_t bitmap[KB_BITMAP_BYTES] = {0};
    for (int i = 2; i < length; i++) {
        uint8_t k = data[i];
        if (k <= 0x03) continue;              // 0=none, 1-3=rollover/error markers
        if (k <= KB_KEY_MAX) {
            bitmap[k >> 3] |= (1 << (k & 7));
        }
    }

    // Forward only when the state actually changed.
    bool changed = (modifiers != lastKeyboardModifiers);
    if (!changed) {
        for (int i = 0; i < KB_BITMAP_BYTES; i++) {
            if (bitmap[i] != lastKeyboardBitmap[i]) {
                changed = true;
                break;
            }
        }
    ...
    memcpy(lastKeyboardBitmap, bitmap, KB_BITMAP_BYTES);

    // Binary frame: [0xAB][modifiers][bitmap * 20]
    uint8_t packet[2 + KB_BITMAP_BYTES];
    packet[0] = KB_MARKER;
    packet[1] = modifiers;
    memcpy(&packet[2], bitmap, KB_BITMAP_BYTES);
